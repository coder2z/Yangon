...
func (options *RunOptions) RpcRun() {
	dir, _ := os.Getwd()
	var err error

	data, err := manager.NewDataSource(xflag.NString("rpc", "xcfg"))
	tools.MustCheck(err)
...
func (options *RunOptions) ApiRun() {
	dir, _ := os.Getwd()
	var err error

	data, err := manager.NewDataSource(xflag.NString("apim", "xcfg"))
	tools.MustCheck(err)
	...
	for rows.Next() {
		...
		// model
		{
			...
			//获取到模板文件
			//模板替换
			modelText, err = tools.ParseTmplFile(filepath.Join(dir, "tmp", options.ProjectName, "modelApi", "model.go.tmpl"), map[string]string{
				"TableFieldList": TableFieldList,
				"ProjectName":    options.ProjectName,
				"TableName":      modelName,
				"tableName":      table,
			...
			tools.MustCheck(err)
			//模板替换文件夹位置
			modelPath := filepath.Join(dir, "internal", "model", "{{table}}")
			modelPath = tools.ReplaceAllData(modelPath, map[string]string{
				"{{table}}":   table,
			})
			//创建文件夹
		...
		//handle
		{
			var handleText string
			//获取到模板文件
			handleText, err = tools.ParseTmplFile(filepath.Join(dir, "tmp", options.ProjectName, "modelApi", "handle.go.tmpl"), map[string]string{
				"ProjectName": options.ProjectName,
				"TableName":   modelName,
				"tableName":   table,
			})
			tools.MustCheck(err)
			//模板替换文件位置
			handlePath := filepath.Join(dir, "internal", "api", "{{version}}", "handle")
			handlePath = tools.ReplaceAllData(handlePath, map[string]string{
				"{{version}}": tools.UnStrFirstToUpper(options.Version),
			})
			//创建文件夹
		...
		//server
		{
			var serverText string
			//获取到模板文件
			serverText, err = tools.ParseTmplFile(filepath.Join(dir, "tmp", options.ProjectName, "modelApi", "server.go.tmpl"), map[string]string{
				"ProjectName": options.ProjectName,
				"TableName":   modelName,
				"tableName":   table,
				"Id":          Id,
			...
			//模板替换文件位置
			//模板替换文件夹位置
			serverPath := filepath.Join(dir, "internal", "services", "{{table}}")
			serverPath = tools.ReplaceAllData(serverPath, map[string]string{
				"{{table}}":   table,
			})
			//创建文件夹
		...
		//registry
		{
			var registryText string
			//获取到模板文件
			registryText, err = tools.ParseTmplFile(filepath.Join(dir, "tmp", options.ProjectName, "modelApi", "registry.go.tmpl"), map[string]string{
				"ProjectName": options.ProjectName,
				"TableName":   modelName,
				"tableName":   table,
				"version":     tools.UnStrFirstToUpper(options.Version),
			...
			tools.MustCheck(err)
			//模板替换文件位置
			registryPath := filepath.Join(dir, "internal", "api", "{{version}}", "registry")
			registryPath = tools.ReplaceAllData(registryPath, map[string]string{
				"{{version}}": tools.UnStrFirstToUpper(options.Version),
			})
			//创建文件夹
		...
		//map
		{
			var mapText string
			//获取到模板文件
			mapText, err = tools.ParseTmplFile(filepath.Join(dir, "tmp", options.ProjectName, "modelApi", "map.go.tmpl"), map[string]string{
				"TableName":     modelName,
				"TableFieldMap": TableFieldMap,
			})
			tools.MustCheck(err)
			//模板替换文件位置
			mapFile := filepath.Join(dir, "internal", "map", "{{table}}.go")
			mapFile = tools.ReplaceAllData(mapFile, map[string]string{
				"{{table}}":   table,
			})
			//判断文件存在，如果存在 就备份之前文件
